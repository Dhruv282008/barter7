...
getRecieverDetails(){
  db.collection('requested_products').where('user2_id','==',this.state.receiverId).get()
  .then(snapshot=>{
    snapshot.forEach(doc=>{
      this.setState({
        recieverName    : doc.data().user2_name,
        recieverContact : doc.data().user2_contact,
        recieverAddress : doc.data().user2_address,
      })
    })
  });

  db.collection('all_requests').where('request_id','==',this.state.exchangeId).get()
  .then(snapshot=>{
    snapshot.forEach(doc => {
...
updateBarterStatus=()=>{
  db.collection('all_Barters').add({
    ...
    request_status      :  "Donor's Interested"
  })
}
